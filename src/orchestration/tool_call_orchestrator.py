...
def _to_openai_tool_specs(tool_names: List[str]) -> List[Dict[str, Any]]:
    ...
    # Virtual UI tool spec for explicit clarifications
    if "ui.ask" in tool_names:
        specs.append({
            "type": "function",
            "function": {
                "name": "ui.ask",
                "description": "Ask the user a question and wait for their response. Use when preferences or missing inputs are needed.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "question": {"type": "string", "description": "Question to ask the user"},
